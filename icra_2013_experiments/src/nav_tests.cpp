...
bool NavTests::sendNavGoal(std::string name)
{
  ...
  goal.target_pose.pose.position.z = loc_map_[exp_map_[name].goal].at(2);
  rpyToQuat(0, 0, loc_map_[exp_map_[name].goal].at(3), goal.target_pose.pose.orientation.x, goal.target_pose.pose.orientation.y, goal.target_pose.pose.orientation.z, goal.target_pose.pose.orientation.w);
...
bool NavTests::runTests()
{
  ...
  for(std::map<std::string,Experiment>::iterator iter = exp_map_.begin(); iter != exp_map_.end(); ++iter)
  {
    ...
    ROS_INFO("start_base: %0.3f %0.3f %0.3frad", bx, by, btheta);
